...
import { CITIES, REGION_LABELS, REGION_ORDER, CITY_COUNTRY } from '../../lib/cities';
...
// CITIES, REGION_LABELS, REGION_ORDER, CITY_COUNTRY imported from ../../lib/cities.js
...
export default function MingPanPage() {
  ...
  // Pre-fill from query params (from home page inline form)
  const autoSubmitRef = useRef(false);
  useEffect(() => {
    if (typeof window === 'undefined') return;
    const params = new URLSearchParams(window.location.search);
    const qBirthday = params.get('birthday');
    const qHour = params.get('hour');
    const qGender = params.get('gender');
    const qLang = params.get('lang');
    const qCity = params.get('city');
    if (qBirthday) setBirthday(qBirthday);
    if (qHour !== null && qHour !== undefined) setHour(parseInt(qHour) || 0);
    if (qGender === '男' || qGender === '女') setGender(qGender);
    if (qLang === 'zh' || qLang === 'en') setLang(qLang);
    // Try to match city and auto-submit
    if (qCity && qBirthday) {
      const cityLower = qCity.toLowerCase();
      const matched = CITIES.find(c => c[0] === qCity || c[1].toLowerCase() === cityLower);
      if (matched) {
        setBirthLoc(matched);
    ...
    // Clean up URL params
    if (qBirthday) window.history.replaceState({}, '', '/mingpan');
  }, []);
  ...
  // Auto-submit when pre-filled from home page
  useEffect(() => {
    if (autoSubmitRef.current && birthday && birthLoc) {
